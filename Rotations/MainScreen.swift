...
// MARK: - Costome functions:
    ...
    func reorderArray() {
        if firstAssignments.count == 0 || firstAssignments.count == 1 {
            return
        }
        for _ in 1 ... firstAssignments.count {
            ...
            while diceRoll1 == diceRoll2 {
                diceRoll1 = Int(arc4random_uniform(UInt32(firstAssignments.count)))
                diceRoll2 = Int(arc4random_uniform(UInt32(firstAssignments.count)))
                if diceRoll1 != diceRoll2 {
                    // Swap elements at index: 2 and 3
                    print(firstAssignments)
                    firstAssignments.swapAt(diceRoll1, diceRoll2)
